...
public class Stem {
	...
	/**
	 * @param stem add stem to HashMap
	 */
	...
	/**
	 * @param stemID remove stem from HashMap
	 */
	...
	public int childStemNumbers(Stem baseStem) {
		...
		for (int i = baseStem.stemID; i < stemMap.size(); i++) {
			/*
			 * if the present stem is base stem it should be the next plant then break the
			 * for loop to return the stem number of the current plant
			 */
	...
	public String traverseHashMap(Stem baseStem) {
		...
		for (int i = baseStem.stemID; i < stemMap.size(); i++) {
			/*
			 * if the present stem is base stem it should be the next plant then break the
			 * for loop to only print the stem of the current plant
			 */
			if (i != baseStem.stemID && isBaseStem(stemMap.get(i)) == true)
				break;
